...
module.exports = {
  ...
  register: async (req, res) => {
    ...
    try {
      ...
      return res.render("admin/registration", {
        error:
          "Something went wrong while registering the user: " + err.message,
        success: null,
      });
    }
  },
  ...
  login: async (req, res) => {
    ...
    try {
      const loginQuery = `
            ...
            `;
      const [rows] = await pool.execute(loginQuery, [
        username,
        username,
        hashedPassword,
      ]);
      ...
      // Redireciona diretamente para o dashboard após login
      return res.redirect("/admin/dashboard");
    } catch (err) {
